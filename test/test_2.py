...
def test_student_output():
    answer_hashes = {
        'apple': 'bc52dd634277c4a34a2d6210994a9a5e2ab6d33bb4a3a8963410e00ca6c15a02',
    ...
    for line in message:
        if ":" in line:
            ...
            student_answers[key.strip()] = value.strip()

    for key, correct_hash in answer_hashes.items():
        assert key in student_answers, f"❌ Wrong Answer"
        student_hash = hashlib.sha256(student_answers[key].encode()).hexdigest()
        assert student_hash == correct_hash, f"❌ Wrong Answer."
